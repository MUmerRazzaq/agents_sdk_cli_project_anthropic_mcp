...
async def main():
    async with MCPClient(
        ...
        for tool in tools:
            print(f"\n\nTool Name: {tool.name}, \n\nDescription: {tool.model_dump_json()}")
        #print("Available Tools:", tools)


        # Call a tool
        tool_result = await _client.call_tool(
            "read_doc_contents", {"doc_id": "deposition.md"}
        )
        print("\n\nTool Result:", tool_result)

        # Test case for edit_document with non-existent old_str
        try:
